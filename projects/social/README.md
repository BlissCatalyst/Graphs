...
Vertexes are people using the social network
Edges are friendships/connections between users - relationships
Connected components - Extended social network
...
- Hint 2: Instead of using a `set` to mark users as visited, you could use a `dictionary`. Similar to sets, checking if something is in a dictionary runs in O(1) time. If the visited user is the key, what would the value be? - friends?
